...
%
% 2. If output choice is declared, 'simple' is used as the default. This
%    will mean no header or parameter record will be outputted.
%
%==========================================================================
% OUTPUTS:
%--------------------------------------------------------------------------
...
%                 D=fread(fid,d,'int8') %parameter dimensions
%                 fseek(fid,-d,'cof');
...
%             ThisC3D.Parameters.(NAMES{abs(N)}).(pName).Description=fread(fid,[1,LD],'*char'); %parameter description
                        
            fseek(fid,next,'bof'); %go to next group/parameter
%             ThisC3D.Parameters.(NAMES{abs(N)}).Description=cellstr(fread(fid,[1,LD],'*char')); %name
        end
    end
...
%         ThisC3D.Markers.Time0=ThisC3D.Markers.Time-ThisC3D.Markers.Time(1);
        if any(gaps(:))
    ...
    if ThisC3D.Parameters.FORCE_PLATFORM.USED > 0
        ...
        X=mean(mean(corners([1,4],:,:))-mean(corners([2,3],:,:)),3);
        X=X/norm(X);
        Y=mean(mean(corners([1,2],:,:))-mean(corners([3,4],:,:)),3);
        Y=Y/norm(Y);
        Z=cross(X,Y);
        R=[X;Y;Z];
        ...
        for p=1:nPlates
            ...
            F=(R*ThisC3D.Analogue.Data(:,ThisC3D.Parameters.FORCE_PLATFORM.CHANNEL(1:3,p))')'; %force
            M=(R*ThisC3D.Analogue.Data(:,ThisC3D.Parameters.FORCE_PLATFORM.CHANNEL(4:6,p))')'+[-ORIGIN(p,3)*F(:,2),ORIGIN(p,3)*F(:,1),zeros(size(F,1),1)]; %moment
    ...
    if length(filenames) == 1
        ...
        f=strfind(ShortName,' ');
        if ~isempty(f) %replace spaces with underscores
            ShortName(f)='_';
        end
        f=strfind(ShortName,'.');
        if ~isempty(f) %replace full stops with underscores
            ShortName(f)='_';
        end
