...
C**** Communicate among processors to refine the neighborhood.
     ...
     1 	            work, M, N, X, XOPT, FOPT, VM, NT, NS, F)
      ...
      if (thiscpu .eq. 1) then
         if (modulo(cycles,nt) .eq. 0) then 
            print*, 'NT cycles: ', cycles, '| FOPT: ', abs(FOPT)
         else 
            print*,'NT cycles: ', cycles
         endif
      endif
      cycles = cycles + 1
...
C========================================================================
     ...
     1                    IER)   
      ...
      double precision XOPT(N),VM(*),FOPT,T
...
1002  FORMAT('%%%%%%%%%% Optimal activation parameters %%%%%%%%%', /,
     &       '%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%', /,
     &       '--------------------------------------------------', /,
     &       'Hip extensor', /,
     &       '--------------------------------------------------', /,
