...
pnamem = 'C:\Users\tomro\SprintingModel\Model\7Seg\GIFs\Matching\';
fname = '7segSprint.1';
datas = importdata([pnames fname], ' ', 8); datas = datas.data;
datat = importdata([pnamet fname], ' ', 8); datat = datat.data;
datam = importdata([pnamem fname], ' ', 8); datam = datam.data;
...
for i = 1:1:n
    ...
    line(datam(i,2:2:16), datam(i,3:2:17), 'Color', col2)    % Stance leg and HAT segments
    line(datam(i,[12 18]), datam(i,[13 19]), 'Color', col2)  % Swing leg
    line(datam(i,[4 8]), datam(i, [5 9]), 'Color', col2)     % Foot
%     plot(datam(i,end-9), datam(i,end-8), [col2 'x'])       % HAT CoM
%     plot(datam(i,end-7), datam(i,end-6), [col2 'x'])       % Stance leg CoM
%     plot(datam(i,end-5), datam(i,end-4), [col2 'x'])       % Swing leg CoM
    plot(datam(i,end-3), datam(i,end-2), [col2 'o'], 'MarkerSize', 4)% CoM
%     line(xlim, [0 0], 'Color', 'k', 'LineWidth', .8)
